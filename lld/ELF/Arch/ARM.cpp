...
bool ARM::needsThunk(RelExpr expr, RelType type, const InputFile *file,
  ...
  case R_ARM_THM_JUMP24:
    ...
    if (expr == R_PLT_PC || (!config->thumbPlt && s.isFunc() && (s.getVA() & 1) == 0))
      return true;
    [[fallthrough]];
  case R_ARM_THM_CALL: {
    uint64_t dst = (expr == R_PLT_PC) ? s.getPltVA() : s.getVA();
...
void ARM::relocate(uint8_t *loc, const Relocation &rel, uint64_t val) const {
  ...
  case R_ARM_THM_CALL: {
    ...
    if (!rel.sym->isFunc() && (config->thumbPlt || !rel.sym->isInPlt()) && isBlx == bit0Thumb)
      stateChangeWarning(loc, rel.type, *rel.sym);
    if (rel.sym->isFunc() || (!config->thumbPlt && rel.sym->isInPlt()) ? !bit0Thumb : isBlx) {
